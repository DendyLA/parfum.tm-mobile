...
function getTextFromHtml(value: string) {
  return normalizeProductHtml(value).replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
}
...
export default function ProductDetailScreen() {
  ...
  const [selectedVariation, setSelectedVariation] = useState<CartVariation | null>(null);
    ...
    async function loadProduct() {
      ...
      try {
        ...
        if (alive) {
          ...
          const firstActiveVariation = data.variations.find((variation) => variation.isActive !== false);
          if (firstActiveVariation) {
            setSelectedVariation({
              id: firstActiveVariation.id,
              label: [firstActiveVariation.typeName, firstActiveVariation.value].filter(Boolean).join(': '),
            });
            setActiveImage(firstActiveVariation.gallery?.[0]?.image || data.image);
          } else {
    ...
    return (
      <SafeAreaView style={styles.root}>
        <View style={styles.loadingWrap}>
          ...
          <ThemedText style={styles.statusText}>Загружаем товар</ThemedText>
        </View>
      </SafeAreaView>
    );
  }
  ...
  if (error || !product) {
    return (
      <SafeAreaView style={styles.root}>
        <View style={styles.loadingWrap}>
          <ThemedText style={styles.statusText}>Товар не найден</ThemedText>
  ...
  function addProductToCart() {
    ...
    Array.from({ length: quantityToAdd }).forEach(() => addItem(product, selectedVariation));
  ...
  function selectVariation(variation: ProductDetail['variations'][number]) {
    ...
    setSelectedVariation({
      id: variation.id,
      label: [variation.typeName, variation.value].filter(Boolean).join(': '),
    });
    ...
    <SafeAreaView style={styles.root}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        <View style={styles.header}>
          ...
          <View style={styles.headerActions}>
            <TouchableOpacity style={styles.headerIconButton} onPress={() => toggleFavorite(product)}>
              <Heart
            ...
            <TouchableOpacity style={styles.headerIconButton} onPress={() => router.push('/cart')}>
              <ShoppingBag color={palette.primary} size={21} strokeWidth={2.1} />
              {totalQuantity ? (
                <View style={styles.cartBadge}>
                  <ThemedText style={styles.cartBadgeText}>{totalQuantity}</ThemedText>
                </View>
              ) : null}
            </TouchableOpacity>
          </View>
        ...
        <View style={styles.imageWrap}>
          {visibleImage ? (
            <Image source={{ uri: visibleImage }} contentFit="contain" style={styles.image} />
          ) : (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.gallery}>
            {galleryImages.map((image) => (
              <TouchableOpacity
                ...
                style={[styles.galleryItem, image.image === visibleImage && styles.galleryItemActive]}
                onPress={() => setActiveImage(image.image)}>
                {image.image ? (
                  <Image source={{ uri: image.image }} contentFit="contain" style={styles.galleryImage} />
                ) : null}
              </TouchableOpacity>
            ))}
          </ScrollView>
        ...
        {product.variations.length ? (
          <View style={styles.section}>
            <ThemedText style={styles.sectionTitle}>Варианты</ThemedText>
              ...
              {product.variations.map((variation) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    variation.isActive === false && styles.variationChipDisabled,
                  ]}
                  ...
                  <ThemedText
                    style={[
                      ...
                      variation.isActive === false && styles.variationTextDisabled,
                    ]}>
                    {[variation.typeName, variation.value].filter(Boolean).join(': ')}
                  </ThemedText>
                </TouchableOpacity>
              ))}
            </View>
      ...
      <View style={styles.bottomBar}>
        <View style={styles.quantityRow}>
          <ThemedText style={styles.quantityLabel}>Количество</ThemedText>
          <View style={styles.quantityControl}>
            <TouchableOpacity
              activeOpacity={0.78}
              disabled={quantityToAdd <= 1}
              style={[styles.quantityButton, quantityToAdd <= 1 && styles.quantityButtonDisabled]}
              onPress={decrementQuantity}>
              <Minus color={palette.primary} size={18} strokeWidth={2.4} />
            </TouchableOpacity>
            <ThemedText style={styles.quantityText}>{quantityToAdd}</ThemedText>
            <TouchableOpacity
              activeOpacity={0.78}
              disabled={quantityToAdd >= maxQuantity}
              style={[styles.quantityButton, quantityToAdd >= maxQuantity && styles.quantityButtonDisabled]}
              onPress={incrementQuantity}>
              <Plus color={quantityToAdd >= maxQuantity ? palette.secondary : palette.primary} size={18} strokeWidth={2.4} />
            </TouchableOpacity>
          </View>
        </View>
        {addedMessage ? (
          <View style={styles.addedNotice}>
            <Check color={palette.success} size={17} strokeWidth={2.3} />
            <ThemedText style={styles.addedNoticeText}>{addedMessage}</ThemedText>
          </View>
        ) : null}
        <TouchableOpacity
          activeOpacity={0.86}
